...
public class DefaultAereoDao implements AereoDao {
	...
	public int insertAereo(AereoModel aereo) {
		...
		int ritorno = 0;
		Connection conn;
		String query = "INSERT IGNORE INTO aereo values('" + aereo.getTipo() + "', " + aereo.getNumeroPosti() + ", " + aereo.getQuantita() + ");";
	...
	public AereoModel selectAereo(String tipo) {
		...
		try {
			...
			if(rs.next()) {
				result = new AereoModel(rs.getString("tipo"), rs.getInt("numeroPosti"), rs.getInt("quantitaMerci"));
			}
